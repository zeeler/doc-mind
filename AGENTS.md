...
页面结构从上到下：
1. **标题栏** — 返回按钮 + "资料管理"
2. **搜索工具栏** — 搜索输入框 + 类型选择(片段/文档) + 搜索按钮 + 状态/分类筛选
3. **上传 & 扫描（并排各半）** — 左:拖拽上传区, 右:选择本地目录按钮（等高同宽）
4. **统计信息栏** — 向量维度/数量 + 任务进度 + 资料类型统计（可折叠）
5. **搜索结果**（有搜索时显示）
6. **检索范围条**（`.scope-bar`，有勾选时显示）— 已选数量 + 清除按钮
7. **文档表格**（`.docs-table`，kotaemon Files 页风格）— 复选框 | 标题 | 类型 | 大小 | 状态 | 块数 | 标签 | 操作；勾选即限定聊天检索范围（`selectedDocs`，存 localStorage `kb_selected_docs`），翻页每页 20 条

## Frontend: 聊天页引用面板

- 消息气泡下的文档引用胶囊可点击 → 右侧滑出 `.cite-panel`（桌面 400px 并排，移动端 fixed 覆盖）
- 面板内容：引用摘录（`GET /documents/{id}/chunks/{no}/context`，目标块高亮）+ 原文预览（`GET /documents/{id}/file`，pdf/文本用 iframe，图片用 img）
- 聊天输入区下方的 `.scope-chip` 显示当前限定范围，可一键清除
...
- Prefer Python for agent logic and orchestration
- Use type hints and pydantic models for data validation
- Keep agent tool definitions and system prompts in dedicated, well-named files
- Use async patterns for I/O-bound operations
- Document agent capabilities in code, not in separate docs
- Default language for comments and responses: Chinese
- Default MLX local models, not cloud APIs
